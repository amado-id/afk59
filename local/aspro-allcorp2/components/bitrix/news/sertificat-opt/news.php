...
<?else:?>
	...
	<?$arItems = CCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y'), false, false, array('ID', 'NAME', 'ACTIVE_FROM', 'FILE'));
	$arYears = array();
			...
			if($arParams['USE_FILTER'] != 'N')
			{
				...
				<div class="head-block top">
					...
					<div class="item-link <?=($bHasYear ? '' : 'active');?>">
						<?if($bHasYear):?>
							<p class="title btn-inline black" href="<?=$arResult['FOLDER'];?>">
						<?else:?>
							<div class="title">
						<?endif;?>
